...
import com.loyaltyworks.wavinseapp.model.CountryList

class CountryAdapter(context: Context, @LayoutRes resource: Int, locationStateCities: List<CountryList>) : ArrayAdapter<CountryList?>(context, resource, locationStateCities as List<CountryList?>) {
    private val locationStateCities: List<CountryList> = locationStateCities
    override fun getItemId(position: Int): Long {
        return position.toLong()
    }

    override fun getItem(position: Int): CountryList? {
    ...
    override fun getView(position: Int, convertView: View?, parent: ViewGroup): View {
        ...
        category.setText(getItem(position)?.CountryName)
